...
var NOTIFICATIONS_FILE = import_path.default.join(process.cwd(), "notifications_local.json");
var WAREHOUSE_CONFIG_FILE = import_path.default.join(process.cwd(), "warehouse_config.json");
function readWarehouseConfig() {
  try {
    if (import_fs.default.existsSync(WAREHOUSE_CONFIG_FILE)) {
      return JSON.parse(import_fs.default.readFileSync(WAREHOUSE_CONFIG_FILE, "utf8"));
    }
  } catch (err) {
...
function saveWarehouseConfig(config) {
  try {
    import_fs.default.writeFileSync(WAREHOUSE_CONFIG_FILE, JSON.stringify(config, null, 2), "utf8");
  } catch (err) {
...
if (import_fs.default.existsSync(VAPID_FILE)) {
  try {
    vapidKeys = JSON.parse(import_fs.default.readFileSync(VAPID_FILE, "utf8"));
  } catch (err) {
    console.error("Error reading stable VAPID file, regenerando...", err);
    vapidKeys = import_web_push.default.generateVAPIDKeys();
    import_fs.default.writeFileSync(VAPID_FILE, JSON.stringify(vapidKeys, null, 2), "utf8");
  }
} else {
  vapidKeys = import_web_push.default.generateVAPIDKeys();
  try {
...
function readPushSubscriptions() {
  try {
    if (import_fs.default.existsSync(SUBSCRIPTIONS_FILE)) {
      return JSON.parse(import_fs.default.readFileSync(SUBSCRIPTIONS_FILE, "utf8"));
    }
  } catch (err) {
...
function readLocalNotifications() {
  try {
    if (import_fs.default.existsSync(NOTIFICATIONS_FILE)) {
      return JSON.parse(import_fs.default.readFileSync(NOTIFICATIONS_FILE, "utf8"));
    }
  } catch (err) {
...
function saveLocalNotifications(notifications) {
  try {
    import_fs.default.writeFileSync(NOTIFICATIONS_FILE, JSON.stringify(notifications, null, 2), "utf8");
  } catch (err) {
...
function readLocalClients() {
  try {
    if (import_fs.default.existsSync(CLIENTS_FILE)) {
      return JSON.parse(import_fs.default.readFileSync(CLIENTS_FILE, "utf8"));
    }
  } catch (err) {
...
function saveLocalClients(clients) {
  try {
    import_fs.default.writeFileSync(CLIENTS_FILE, JSON.stringify(clients, null, 2), "utf8");
  } catch (err) {
...
function getDeletedClientKeys() {
  ...
  try {
    const pathsToTry = ["deleted_clients.json", "/tmp/deleted_clients.json", import_path.default.join(process.cwd(), "deleted_clients.json")];
...
function addDeletedClientKeys(...keys) {
  ...
  const pathsToSave = ["deleted_clients.json", "/tmp/deleted_clients.json", import_path.default.join(process.cwd(), "deleted_clients.json")];
  pathsToSave.forEach((fp) => {
    try {
      import_fs.default.writeFileSync(fp, JSON.stringify(arr, null, 2));
    } catch (e) {
    }
  });
}
...
var PAYMENTS_FILE = import_path.default.join(process.cwd(), "payments_local.json");
function readLocalPayments() {
  try {
    if (import_fs.default.existsSync(PAYMENTS_FILE)) {
      return JSON.parse(import_fs.default.readFileSync(PAYMENTS_FILE, "utf8"));
    }
  } catch (err) {
...
function saveLocalPayments(payments) {
  try {
    import_fs.default.writeFileSync(PAYMENTS_FILE, JSON.stringify(payments, null, 2), "utf8");
  } catch (err) {
...
async function syncInvoiceToPermanentBackup(id, invoiceObj) {
  ...
  try {
    const backupPath = import_path.default.join(process.cwd(), "invoices_permanent_backup.json");
    let invoicesList = [];
    if (import_fs.default.existsSync(backupPath)) {
      try {
        invoicesList = JSON.parse(import_fs.default.readFileSync(backupPath, "utf8"));
      } catch (e) {
    ...
    if (invoiceData) {
      ...
      import_fs.default.writeFileSync(backupPath, JSON.stringify(invoicesList, null, 2), "utf8");
...
async function syncPaymentToPermanentBackup(id, paymentObj) {
  if (!JSON_BACKUP_ENABLED) return;
  try {
    const backupPath = import_path.default.join(process.cwd(), "payments_permanent_backup.json");
    let paymentsList = [];
    if (import_fs.default.existsSync(backupPath)) {
      try {
        paymentsList = JSON.parse(import_fs.default.readFileSync(backupPath, "utf8"));
      } catch (e) {
    ...
    if (paymentData) {
      ...
      import_fs.default.writeFileSync(backupPath, JSON.stringify(paymentsList, null, 2), "utf8");
...
app.post("/api/app-logo/upload", requireAuth, requireAdmin, upload.single("logo"), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const ext = req.file.originalname ? import_path.default.extname(req.file.originalname) : ".png";
...
app.post("/api/products/:id/image", requireAuth, requireAdmin, upload.single("image"), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      contentType = req.file.mimetype;
      const ext = req.file.originalname ? import_path.default.extname(req.file.originalname) : ".jpg";
...
app.get("/api/offers", requireAuth, asyncHandler(async (req, res) => {
  ...
  offers = offers || [];
  try {
    if (import_fs.default.existsSync("offers_extra.json")) {
      const extra = JSON.parse(import_fs.default.readFileSync("offers_extra.json", "utf-8"));
      offers.forEach((o) => {
        if (extra[o.id]) {
...
app.post("/api/offers", requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  ...
  try {
    let extra = {};
    if (import_fs.default.existsSync("offers_extra.json")) {
      extra = JSON.parse(import_fs.default.readFileSync("offers_extra.json", "utf-8"));
    }
    extra[id] = { price: offerPrice, sellerPrices: offerSellerPrices };
    import_fs.default.writeFileSync("offers_extra.json", JSON.stringify(extra));
  } catch (e) {
  }
...
app.delete("/api/sales/clear", requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  try {
    ...
    try {
      ...
      import_fs.default.writeFileSync(import_path.default.join(backupsDir, backupFileName), JSON.stringify(backupData, null, 2), "utf8");
...
app.get("/api/invoices/folio-config", requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  ...
  if (import_fs.default.existsSync(FOLIO_CONFIG_FILE)) {
    try {
      folioConfig = JSON.parse(import_fs.default.readFileSync(FOLIO_CONFIG_FILE, "utf-8"));
    } catch (err) {
    }
  }
  try {
...
app.post("/api/invoices/reset-folio", requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  ...
  const FOLIO_CONFIG_FILE = import_path.default.join(process.cwd(), "folio_config.json");
  try {
    import_fs.default.writeFileSync(FOLIO_CONFIG_FILE, JSON.stringify(config, null, 2), "utf8");
  } catch (err) {
  }
  try {
...
app.get("/api/inventory/excluded-critical", requireAuth, asyncHandler(async (req, res) => {
  let excludedIds = [];
  const EXCLUDED_FILE = import_path.default.join(process.cwd(), "excluded_critical.json");
  if (import_fs.default.existsSync(EXCLUDED_FILE)) {
    try {
      excludedIds = JSON.parse(import_fs.default.readFileSync(EXCLUDED_FILE, "utf-8"));
    } catch (err) {
    }
  }
  try {
...
app.post("/api/inventory/excluded-critical", requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  ...
  const EXCLUDED_FILE = import_path.default.join(process.cwd(), "excluded_critical.json");
  try {
    import_fs.default.writeFileSync(EXCLUDED_FILE, JSON.stringify(list, null, 2), "utf8");
  } catch (err) {
  }
  try {
...
app.get("/api/invoices/print-template", requireAuth, asyncHandler(async (req, res) => {
  let template = "";
  const TEMPLATE_FILE = import_path.default.join(process.cwd(), "print_template.txt");
  if (import_fs.default.existsSync(TEMPLATE_FILE)) {
    try {
      template = import_fs.default.readFileSync(TEMPLATE_FILE, "utf-8");
    } catch (err) {
    }
  }
  if (!template) {
...
app.post("/api/invoices/print-template", requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  ...
  const TEMPLATE_FILE = import_path.default.join(process.cwd(), "print_template.txt");
  try {
    import_fs.default.writeFileSync(TEMPLATE_FILE, template || "", "utf8");
  } catch (err) {
  }
  try {
...
var debtsFile = import_path.default.resolve(process.cwd(), "business-debts.json");
var readDebts = async () => {
  try {
    ...
    if (!error && data && data.photo) {
      ...
      if (Array.isArray(parsed)) {
        try {
          import_fs.default.writeFileSync(debtsFile, data.photo, "utf-8");
        } catch {
        }
        return parsed;
      }
  ...
  try {
    if (import_fs.default.existsSync(debtsFile)) {
      const parsed = JSON.parse(import_fs.default.readFileSync(debtsFile, "utf-8"));
...
var writeDebts = async (data) => {
  ...
  try {
    import_fs.default.writeFileSync(debtsFile, payloadStr, "utf-8");
  } catch (e) {
...
var suppliersFile = import_path.default.resolve(process.cwd(), "suppliers.json");
var readSuppliers = async () => {
  ...
  try {
    ...
    if (!error && data && data.photo) {
      ...
      if (Array.isArray(parsed)) {
        try {
          import_fs.default.writeFileSync(suppliersFile, data.photo, "utf-8");
        } catch {
        }
        return parsed;
      }
  ...
  try {
    if (import_fs.default.existsSync(suppliersFile)) {
      const parsed = JSON.parse(import_fs.default.readFileSync(suppliersFile, "utf-8"));
      if (Array.isArray(parsed)) return parsed;
    }
  } catch (e) {
...
var writeSuppliers = async (data) => {
  const payloadStr = JSON.stringify(data, null, 2);
  try {
    import_fs.default.writeFileSync(suppliersFile, payloadStr, "utf-8");
  } catch (e) {
  ...
  localFiles.forEach((file) => {
    try {
      ...
      if (import_fs.default.existsSync(filePath)) {
        const arr = JSON.parse(import_fs.default.readFileSync(filePath, "utf-8"));
...
function readLocalRecibosCaja() {
  try {
    if (import_fs.default.existsSync(RECIBOS_CAJA_FILE)) {
      const raw = import_fs.default.readFileSync(RECIBOS_CAJA_FILE, "utf-8");
...
function saveLocalReciboCaja(recibo) {
  try {
    ...
    list.unshift(recibo);
    import_fs.default.writeFileSync(RECIBOS_CAJA_FILE, JSON.stringify(list, null, 2));
  } catch (e) {
