...
with open('db_items.json', 'r') as f:
...
for line in text.strip().split('\n'):
    match = re.search(r'(.*?)\s+(\d+)\s+Q([\d,.]+)\s+Q([\d,.]+)', line)
    if match:
        name = match.group(1).strip().lower()
        val = float(match.group(4).replace(',', ''))
        excel_items[name] = val

print("Comparing...")
missing_in_db = []
diffs = []
for name, val in excel_items.items():
    if name not in db_items:
        # try fuzzy match
        found = False
        for db_name, db_val in db_items.items():
            if name in db_name or db_name in name:
                if abs(val - db_val) > 0.01:
                    diffs.append((name, val, db_name, db_val))
                found = True
                break
        if not found:
            missing_in_db.append((name, val))
    else:
        if abs(val - db_items[name]) > 0.01:
            diffs.append((name, val, name, db_items[name]))

print("Missing in DB completely:", missing_in_db)
print("Differences:", diffs)

missing_in_excel = []
for name, val in db_items.items():
    # see if exact or fuzzy matched
    found = False
    if name in excel_items:
        found = True
    else:
        for ex_name, ex_val in excel_items.items():
            if name in ex_name or ex_name in name:
                found = True
                break
    if not found:
        missing_in_excel.append((name, val))

print("Missing in EXCEL completely (app has it but Excel doesn't):")
for n, v in missing_in_excel:
    print(n, v)
