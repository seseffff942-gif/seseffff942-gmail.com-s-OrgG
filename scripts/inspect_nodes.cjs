...
db.get('SELECT executionId, data FROM execution_data ORDER BY executionId DESC LIMIT 1', (err, row) => {
  if (err || !row) return console.error(err || 'No row found');
  try {
    ...
    if (runData['Evaluar Hora y Meta']) {
      console.log('Eval output:', JSON.stringify(runData['Evaluar Hora y Meta'][0].data.main[0][0].json, null, 2));
    }
  } catch(e) {
